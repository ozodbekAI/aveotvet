...
def load_prompt_prefs(s: ShopSettings) -> PromptPrefs:
    ...
    address_format = _get_str(adv, "address_format", DEFAULT_PREFS.address_format)
    if address_format not in ("vy_caps", "vy_lower", "ty"):
        address_format = DEFAULT_PREFS.address_format

    answer_length = _get_str(adv, "answer_length", DEFAULT_PREFS.answer_length)
    if answer_length not in ("short", "default", "long"):
        answer_length = DEFAULT_PREFS.answer_length
...
def pick_signature(
    ...
    *,
    ...
    """Pick a signature from ShopSettings.signatures.

    Backward compatible:
    - list[str] is supported
    - list[dict] is supported (UI-like), example:
      {"text":"...","type":"review","brand":"all","created_at":"2026-01-06T...Z"}
    """
