...
async def _job_sync_shop(session: AsyncSession, payload: dict) -> None:
    ...
    # enqueue drafts for unanswered feedbacks if enabled
...
async def _job_generate_draft(session: AsyncSession, payload: dict) -> None:
    ...
    text, model, response_id = await generate_draft_text(openai, feedback, settings_obj)
...
async def _job_generate_chat_draft(session: AsyncSession, payload: dict) -> None:
    ...
    openai = OpenAIService()
    text, model, rid = await generate_chat_reply(openai, settings_obj, last_text, context=ctx)
